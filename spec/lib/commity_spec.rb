...
RSpec.describe Commity do
  it 'loads core service constants' do
    ...
    expect(defined?(Commity::GoogleClient)).to eq('constant')
    expect(defined?(Commity::DiffParser)).to eq('constant')
    ...
    expect(defined?(Commity::Flows::CommitFlow)).to eq('constant')
    expect(defined?(Commity::Flows::PrFlow)).to eq('constant')
  end
end
