...
module Commity
  module InteractivePrompt
    ...
    def self.ask_yes_no(question, default: :no)
      ...
      return true if ["y", "yes"].include?(value)
      return false if ["n", "no"].include?(value)
    ...
    def self.ask_commit_action
      ...
      value = input.strip.downcase
      return :yes if ["y", "yes"].include?(value)
      return :edit if ["e", "edit"].include?(value)
    ...
    def self.editor_command
      ...
      if code_editor_command?(command.first) && !command.include?("--wait")
        command << "--wait"
      end
